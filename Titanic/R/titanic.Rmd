...
glm.fit = glm( Survived~pclass+Sex+FixedAge+FarePerPassenger+FixedTitle+CabinAssignment+Embarked+NumPassengersOnTicket+NumParents+NumChildren+SpousesFactor+Siblings,data=newTrain,family=binomial);
summary( glm.fit );
...
```{r knnTest, echo=FALSE, warning=FALSE}
library(caret);
set.seed(1);
idxTrain = createDataPartition(y=newTrain$Survived,p=0.8,list=FALSE);
trainTrain = newTrain[idxTrain,];
testTrain = newTrain[-idxTrain,];
trainCtrl = trainControl( method="repeatedcv", number=8, repeats = 30);
knn_fit = train( SurvivedFactor~pclass+Sex+FixedAge+FarePerPassenger+FixedTitle+CabinAssignment+Embarked+NumPassengersOnTicket+NumParents+NumChildren+SpousesFactor+Siblings,data=trainTrain,method="knn", trControl=trainCtrl, preProcess=c("center","scale"), tuneLength=20 );
test_pred = predict( knn_fit, newdata=testTrain);
#confusionMatrix( test_pred, testTrain$SurvivedFactor)
```
...
knnTrain = newTrain;
knnTest = newTest;
trainCtrl = trainControl( method="repeatedcv", number=8, repeats = 100);
...
knnModel = data.frame(PassengerId=knnTest$PassengerId, Survived=as.numeric(knn_pred)-1);
write.csv( knnModel, "knnModel.csv", row.names=FALSE, quote=FALSE );
```

KNN Score is 0.76555

### Decision Tree Model
```{r treeModel, echo=FALSE, warning=FALSE}
library(tree);
...
```{r baggingModel, echo=FALSE, warning=FALSE}
...
### Random Forest Model
```{r rfModel, echo=FALSE, warning=FALSE}
library(randomForest);
set.seed(1);
...
```{r boostedModel, echo=FALSE, warning=FALSE}
library(gbm);
set.seed(1);
boosted_model = gbm( Survived~pclass+Sex+FixedAge+FarePerPassenger+FixedTitle+CabinAssignment+Embarked+NumPassengersOnTicket+NumParents+NumChildren+SpousesFactor+Siblings,data=newTrain, distribution="bernoulli", n.trees=5000, interaction.depth=4 );
