...
```{r logisticRegression1, echo=FALSE, warning=FALSE}
...
```{r logisticRegression4, echo=FALSE, warning=FALSE}
library(boot);
...
```{r logisticRegression5, echo=FALSE, warning=FALSE}
correct = list();
accuracies = list();
errors = list();
features = list();
...
```{r logisticRegression6, echo=FALSE, warning=FALSE}
...
write.csv( bestLogisticRegressionModel, "BestLogisticRegressionModel.csv", row.names=FALSE, quote=FALSE );
...
```{r ridgeRegression1, echo=FALSE, warning=FALSE}
library( glmnet );
lambdas = c();
set.seed(1);
...
write.csv( fullRidgeRegressionModel, "FullRidgeRegressionModel.csv", row.names=FALSE, quote=FALSE );
# score is 0.78947
```
...
```{r ridgeRegression1b, echo=FALSE}
confusionMatrix(newTrain$Survived,trainRidgePredictions);
```
...
confusionMatrix( Logistic=fullLogisticPredictions, Ridge=fullRidgePredictions );
```
...
```{r ridgeRegression3, echo=FALSE, warning=FALSE}
library( glmnet );
lambdas = c();
set.seed(1);
# use ridge regression
...
write.csv( bestRidgeRegressionModel, "BestRidgeRegressionModel.csv", row.names=FALSE, quote=FALSE );
```
...
```{r ridgeRegression4, echo=FALSE}
fullLogisticPredictions = bestTestPredictions;
confusionMatrix( Logistic=fullLogisticPredictions, Ridge=bestRidgePredictions );
```
...
write.csv( fullLassoRegressionModel, "FullLassoRegressionModel.csv", row.names=FALSE, quote=FALSE );
```

The Lasso model using all of the features had a training set error of 0.1638608 with a lambda of 0.00160114 (see confusion matrix below).  

```{r lassoRegression2, echo=FALSE, warning=FALSE}
confusionMatrix( newTrain$Survived, trainLassoPredictions );
```
...
```{r lassoRegression3, echo=FALSE, warning=FALSE}
confusionMatrix( Logistic=fullLogisticPredictions, Lasso=lassoPredictions );
```
...
```{r lassoRegression4, echo=FALSE, warning=FALSE}
# on to the lasso
xTrain = model.matrix( Survived~.,data=best_df,family=binomial);
best_df_test = newTest[, best_features];
xTest = model.matrix( ~.,data=best_df_test,family=binomial);
...
write.csv( bestLassoRegressionModel, "BestLassoRegressionModel.csv", row.names=FALSE, quote=FALSE );
```
...
```{r lassoRegression5, echo=FALSE, warning=FALSE}
confusionMatrix( newTrain$Survived, trainLassoPredictions );
```
...
```{r lassoRegression6, echo=FALSE, warning=FALSE}
confusionMatrix( Logistic=fullLogisticPredictions, Lasso=lassoPredictions );
```
...
```{r lassoRegression7, echo=FALSE, warning=FALSE}
confusionMatrix( Logistic=logisticPredictions, Lasso=lassoPredictions );
confusionMatrix( Ridge=bestRidgePredictions, Lasso=lassoPredictions );
# score is 0.78947
```

### K-Nearest Neighbors model
```{r knn1, echo=FALSE, warning=FALSE}
library(caret);
#trainCtrl = trainControl( method="repeatedcv", number=8, repeats = 30);
...
confusionMatrix( newTrain$SurvivedFactor, test_pred)
```
...
write.csv( fullKNNModel, "fullKNNModel.csv", row.names=FALSE, quote=FALSE );
```

KNN Score is 0.76555.  Let's try to use cross validation to choose a better model

```{r knn3, echo=FALSE, warning=FALSE}
# now the whole training set
significant_features = c("FixedTitle", "Sex", "Siblings", "FixedAge", "CabinAssignment", "pclass", "NumChildren", "FarePerPassenger", "Embarked");
all_combinations_of_significant_features = lapply(1:length(significant_features), function(x) combn(length(significant_features),x));
numFeatureSetClasses = length( all_combinations_of_significant_features );
...
trainCtrl = trainControl( method="repeatedcv", number = 8, repeats = 3 );
for ( featureClass in 1:numFeatureSetClasses){
  featClassColumns = dim( all_combinations_of_significant_features[[featureClass]] )[2];
  for ( thisCol in 1:featClassColumns ){
    feature_set = significant_features[ all_combinations_of_significant_features[[featureClass]][,thisCol]];
...
```{r knn4, echo=FALSE, warning=FALSE}
# now the whole training set
correct = list();
accuracies = list();
errors = list();
...
```{r knn5, echo=FALSE, warning=FALSE}
# best 9
knnTrain = newTrain;
knnTest = newTest;
trainCtrl = trainControl( method="LOOCV" );
...
write.csv( best9KNNModel, "best9KNNModel.csv", row.names=FALSE, quote=FALSE );
```
9 scored 0.76076

```{r knn6, echo=FALSE, warning=FALSE}
# best 8
trainCtrl = trainControl( method="LOOCV" );
...
write.csv( best8KNNModel, "best8KNNModel.csv", row.names=FALSE, quote=FALSE );
```
8 scored 0.73684

```{r knn7, echo=FALSE, warning=FALSE}
# best 7
trainCtrl = trainControl( method="LOOCV" );
...
write.csv( best7KNNModel, "best7KNNModel.csv", row.names=FALSE, quote=FALSE );
```

7 scored 0.73684
```{r knn8, echo=FALSE, warning=FALSE}
# best 6
trainCtrl = trainControl( method="LOOCV" );
...
write.csv( best6KNNModel, "best6KNNModel.csv", row.names=FALSE, quote=FALSE );
```
6 scored 0.71770

### Decision Tree Models
```{r treeModel1, echo=FALSE, warning=FALSE}
library(tree);
...
tree_pred = predict( tree_model, newTest, type="class");
...
```
scored 0.80382, best so far

#### Tree model with significant features
```{r treeModel2, echo=FALSE, warning=FALSE}
library(tree);
...
tree_pred = predict( tree_model, newTest, type="class");
treeModel = data.frame(PassengerId=knnTest$PassengerId, Survived=as.numeric(tree_pred)-1);
write.csv( treeModel, "sigTreeModel.csv", row.names=FALSE, quote=FALSE );
```
Scored 0.77990

### Bootstrap Aggregation (Bagging) Tree Model
```{r baggingModel1, echo=FALSE, warning=FALSE}
...
```{r baggingModel2, echo=FALSE, warning=FALSE}
library(randomForest);
set.seed(1);
bag_model = randomForest( SurvivedFactor~pclass+Sex+FixedAge+FarePerPassenger+FixedTitle+CabinAssignment+Embarked+NumChildren+Siblings,data=newTrain, mtry=12, importance=TRUE );
bag_model
plot( bag_model )
bag_pred = predict( bag_model, newdata=newTest);
bagModel = data.frame(PassengerId=newTest$PassengerId, Survived=as.numeric(bag_pred)-1);
write.csv( bagModel, "sigBagModel.csv", row.names=FALSE, quote=FALSE );
# scored 0.74641
```

### Random Forest Model
```{r rfModel1, echo=FALSE, warning=FALSE}
library(randomForest);
set.seed(1);
rf_model = randomForest( SurvivedFactor~pclass+Sex+FixedAge+FarePerPassenger+FixedTitle+CabinAssignment+Embarked+NumPassengersOnTicket+NumParents+NumChildren+SpousesFactor+Siblings,data=newTrain, importance=TRUE );
rf_model
importance( rf_model );
varImpPlot( rf_model );
plot( rf_model )
...
### Random Forest Model
```{r rfModel2, echo=FALSE, warning=FALSE}
library(randomForest);
set.seed(1);
rf_model = randomForest( SurvivedFactor~pclass+Sex+FixedAge+FarePerPassenger+FixedTitle+CabinAssignment+Embarked+NumChildren+Siblings,data=newTrain, importance=TRUE );
rf_model
importance( rf_model );
varImpPlot( rf_model );
plot( rf_model )
rf_pred = predict( rf_model, newdata=newTest);
rfModel = data.frame(PassengerId=newTest$PassengerId, Survived=as.numeric(rf_pred)-1);
write.csv( rfModel, "SigRandomForestModel.csv", row.names=FALSE, quote=FALSE );
# scored 0.76555
```

#### Random Forest Model
```{r rfModel3, echo=FALSE, warning=FALSE}
library(randomForest);
set.seed(1);
rf_model = randomForest( SurvivedFactor~pclass+Sex+FixedAge+FarePerPassenger+FixedTitle+CabinAssignment+Embarked+Siblings,data=newTrain, importance=TRUE );
rf_model
importance( rf_model );
varImpPlot( rf_model );
plot( rf_model )
rf_pred = predict( rf_model, newdata=newTest);
rfModel = data.frame(PassengerId=newTest$PassengerId, Survived=as.numeric(rf_pred)-1);
write.csv( rfModel, "TrimmedRandomForestModel.csv", row.names=FALSE, quote=FALSE );
# scored 0.78947
```

#### Random Forest Model
```{r rfModel4, echo=FALSE, warning=FALSE}
library(randomForest);
set.seed(1);
rf_model = randomForest( SurvivedFactor~pclass+Sex+FixedAge+FarePerPassenger+FixedTitle+Siblings,data=newTrain, importance=TRUE );
rf_model
importance( rf_model );
varImpPlot( rf_model );
plot( rf_model )
rf_pred = predict( rf_model, newdata=newTest);
rfModel = data.frame(PassengerId=newTest$PassengerId, Survived=as.numeric(rf_pred)-1);
write.csv( rfModel, "MinimalRandomForestModel.csv", row.names=FALSE, quote=FALSE );
# scored 0.77511
```

### Boosted Classification Tree Model
```{r boostedModel1, echo=FALSE, warning=FALSE}
library(gbm);
set.seed(1);

xTrain = model.matrix( SurvivedFactor~pclass+Sex+FixedAge+FarePerPassenger+FixedTitle+CabinAssignment+Embarked+NumPassengersOnTicket+NumParents+NumChildren+SpousesFactor+Siblings,data=newTrain,family=binomial);
xTest = model.matrix( ~pclass+Sex+FixedAge+FarePerPassenger+FixedTitle+CabinAssignment+Embarked+NumPassengersOnTicket+NumParents+NumChildren+SpousesFactor+Siblings,data=newTest,family=binomial);
boosted_model = gbm( formula=SurvivedFactor~pclass+Sex+FixedAge+FarePerPassenger+FixedTitle+CabinAssignment+Embarked+NumPassengersOnTicket+NumParents+NumChildren+SpousesFactor+Siblings,data=newTrain, distribution="bernoulli", n.trees=5000, interaction.depth=1, cv.folds=10 );
#summary( boosted_model );
#plot( boosted_model );
boosted_pred = predict( boosted_model, newdata=newTest, n.trees=5000, type="response");
# probabilities >= 0.5 mean survived, < 0.5 mean perished
threshold = 0.5;
...
# scored 0.76076 with interaction depth =4, 0.77990 with interaction depth = 1
```

```{r boostedModel2, echo=FALSE, warning=FALSE}
library(gbm);
set.seed(1);
boosted_model = gbm( Survived~pclass+Sex+FixedAge+FarePerPassenger+FixedTitle+CabinAssignment+Embarked+NumChildren+Siblings,data=newTrain, distribution="bernoulli", n.trees=5000, interaction.depth=4 );
#summary( boosted_model );
#plot( boosted_model );
boosted_pred = predict( boosted_model, newdata=newTest, n.trees=5000, type="response");
# probabilities >= 0.5 mean survived, < 0.5 mean perished
...
boosted_predictions = as.numeric( boosted_pred >= threshold );
boostedModel = data.frame(PassengerId=newTest$PassengerId, Survived=boosted_predictions);
write.csv( boostedModel, "SigBoostedTreeModel.csv", row.names=FALSE, quote=FALSE );
# scored 0.76076
```
### Cheating Model

```{r cheatModel, echo=FALSE, warning=FALSE}
library(RCurl);
library(XML);
library(readr);
